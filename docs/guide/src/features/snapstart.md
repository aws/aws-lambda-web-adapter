...
A hook path must not resolve to a route containing a percent sign, and must not contain a malformed `%` escape (a trailing `%`, or `%zz`). Web frameworks disagree on how to route such a path — some reject it outright, others decode it leniently — so the adapter cannot guarantee it blocks every spelling that reaches the route, and a partially protected hook path is worse than an obviously invalid one. The adapter rejects these values at startup: initialization fails with an error naming the offending path, rather than running with a state-mutating route left reachable. Use a plain path with no percent-encoding.
