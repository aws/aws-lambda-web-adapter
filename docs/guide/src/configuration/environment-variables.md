...
| `AWS_LWA_READINESS_CHECK_TIMEOUT_SECONDS` | Seconds the adapter waits for the app to report ready (cold-start init **and** after a SnapStart restore) before giving up. Unset = wait indefinitely. The `async_init` path keeps its own ~9.8s bound and is unaffected. | unset (unbounded) |
